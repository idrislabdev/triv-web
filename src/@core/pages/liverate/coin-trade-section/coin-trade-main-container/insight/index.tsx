...
const CoinTradeInsight = (props: { asset: any; lang: string }) => {
  const { asset, lang } = props;
  const [moneyInflow, setMoneyInflow] = useState({} as any);
  const [longShort, setLongShort] = useState({} as any);

  const fetchData = useCallback(async () => {
    const resp = await axios.get(
      `https://ins.triv.id/api/v1/asset-insights?currency=${asset.code}&session=market_insight`
    );
  ...
  return (
    <div className="insight-area">
      ...
      <div className="row-2">
        {longShort.long_short_ratio_chart && (
          <RatioChart rawChart={longShort} />
        )}
      </div>
    </div>
  );
