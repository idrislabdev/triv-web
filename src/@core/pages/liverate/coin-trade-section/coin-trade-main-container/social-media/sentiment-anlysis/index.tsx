import { BarChartIcon, ChevronUpIcon } from '@/@core/components/custom-icons';
...
const SentimenAnalysis = (props: { data: any; objLang: any }) => {
  ...
  useEffect(() => {
    function handleClick(event: MouseEvent) {
      if (
        dropdownSentiment.current &&
        !dropdownSentiment.current.contains(event.target)
      ) {
        setShow(false);
      }
    }
        ...
        <div className="card-title">
          ...
          <div className="action-list">
            <div className="dropdown-menu" ref={dropdownSentiment}>
              <a onClick={(_) => setShow(!show)}>
                {medias.find((x) => x.val == selected)?.name} ▼
              </a>
              <ul className={show ? `show` : ``}>
                <li>
                  <a
                    onClick={() => chooseSelected('tweet')}
                    className={selected == 'tweet' ? 'active' : ''}
                  >
                    X
                  </a>
                </li>
                <li>
                  <a
                    onClick={() => chooseSelected('tiktok-video')}
                    className={selected == 'tiktok-video' ? 'active' : ''}
                  >
                    Tiktok
                  </a>
                </li>
                <li>
                  <a
                    onClick={() => chooseSelected('reddit-post')}
                    className={selected == 'reddit-post' ? 'active' : ''}
                  >
                    Reddit
                  </a>
                </li>
                <li>
                  <a
                    onClick={() => chooseSelected('youtube-video')}
                    className={selected == 'youtube-video' ? 'active' : ''}
                  >
                    You Tube
                  </a>
                </li>
