      ...
      <div className="card-body">
        <div className="row">
          ...
          <div className="row-content">
            <label>
              IDR
              {dataPerformance.quote && dataPerformance.quote.IDR
                ? formatDecimalSmart(dataPerformance.quote.IDR.low)
                : 0}
            </label>
            <p>
              IDR
              {dataPerformance.quote && dataPerformance.quote.IDR
                ? formatDecimalSmart(dataPerformance.quote.IDR.high)
                : 0}
            </p>
          </div>
        ...
        <div className="row">
          <div className="row-content">
            ...
            <p>
              IDR
              {dataAllTime.quote && dataAllTime.quote.IDR
                ? formatDecimalSmart(dataAllTime.quote.IDR.high)
                : 0}
            </p>
          </div>
          <div className="row-content child">
            <label>
              {moment(dataPerformance.high_timestamp).format('MMM DD, YYYY')} (
              {moment(dataPerformance.high_timestamp).fromNow()})
            </label>
            <p
              className={`${
                dataAllTime.quote.IDR.price_change > 0
                  ? 'text-plus'
                  : 'text-minus'
              }`}
            >
        ...
        <div className="row">
          <div className="row-content">
            ...
            <p>
              IDR
              {dataAllTime.quote && dataAllTime.quote.IDR
                ? formatDecimalSmart(dataAllTime.quote.IDR.low)
                : 0}
            </p>
          </div>
          <div className="row-content child">
            <label>
              {moment(dataPerformance.low_timestamp).format('MMM DD, YYYY')} (
              {moment(dataPerformance.high_timestamp).fromNow()})
            </label>
            <p
              className={`${
                dataAllTime.quote.IDR.percent_change > 0
                  ? 'text-plus'
                  : 'text-minus'
              }`}
            >
