...
const CoinTradeNews = (props: { asset: any; lang: string; blogs: IBlog[] }) => {
  const { asset, lang, blogs } = props;
  const [newsData, setNewsData] = useState([]);
  const fetchData = useCallback(async () => {
    const resp = await axios.get(
      `https://ins.triv.id/api/v1/asset-insights?currency=${asset.code}&session=news`
    );
