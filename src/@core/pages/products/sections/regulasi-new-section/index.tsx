...
const ProductsRegulasiNewSection = (props: {objLang:any}) => {
  const { objLang } = props
  const inViewport = () => {
    const elements = document.getElementsByClassName("products-regulasi-new-section");
    var element = elements[0];
    if (element) {
      const { top, bottom } = element.getBoundingClientRect();
      if(window.innerHeight > top && bottom > 0) {
        document.getElementsByClassName("products-regulasi-new-section")[0].classList.add('animate')
      } else {
        document.getElementsByClassName("products-regulasi-new-section")[0].classList.remove('animate')
      }
    }
  }
  ...
  return (
    <section className='products-regulasi-new-section'>
