...
import {
  getAsset,
  getBlogs,
  getLiverateHighlights,
  getLiverateHighlightsBySlug,
  getLiverateMini,
  getLiverates,
...
export default async function LiverateCoinTrade({ params }: any) {
  ...
  const resAsset = await getAsset(coin.code);
  const asset: any = resAsset.data.data;

  const { liverate_coin_section } = await getDictionaryLierate(params.lang);
      ...
      <head>
        {params.lang === "id" && (
          <>
            ...
            />
            <meta
              content={`Grafik Harga ${asset.label} Terupdate Hari Ini (${asset.currency} to IDR) - TRIV`}
              name="title"
            />
            <meta
              content={`Jual beli ${asset.label} 24 jam, instant ke 61 bank di Indonesia &amp; spread terendah. Layanan jual beli coin SHIB terpercaya sejak 2015, terdaftar di Bappebti`}
              name="description"
            />
            ...
            />
            <meta
              content={`https://triv.co.id/id/home/liverate/${asset.label.toLowerCase()}`}
              property="og:url"
            />
            <meta
            ...
            />
            <link
              href={`https://triv.co.id/id/home/liverate/${asset.label.toLowerCase()}`}
              rel="canonical"
            />
            ...
            />
            <link
              href={`https://triv.co.id/en/home/liverate/${asset.label.toLowerCase()}`}
              hrefLang="en"
              rel="alternate"
            />
            <link
        ...
        {params.lang === "en" && (
          <>
            ...
            />
            <meta
              content={`Today's Updated ${asset.label} Price Chart (${asset.currency} to IDR) - TRIV`}
              name="title"
            />
            <meta
              content={`${asset.label} price to rupiah today (${asset.currency} to IDR). Check the latest and updated (real time) Shib rate chart only on TRIV, the no.1 crypto marketplace" name="description`}
              name="description"
            />
            <meta content="origin-when-crossorigin" name="referrer" />
            <meta content="index, follow, noodp" name="robots" />
            ...
            />
            <meta
              content={`https://triv.co.id/en/home/liverate/${asset.label.toLowerCase()}`}
              property="og:url"
            />
            <meta
              content="https://triv.co.id/assets/logo_triv-6bfd8b14aab606f32abb737168e7d6ce14c567877614d274e9675d768da1a505.png"
